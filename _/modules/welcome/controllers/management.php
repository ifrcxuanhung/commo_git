...
class Management extends Welcome {
    ...
    function index() {
		...
		$this->data->blockmanagement = $this->db->query("SELECT * FROM simul_settings ss WHERE ss.type='menu' AND ss.stype = 'management' AND ss.active <= ".$this->session->userdata('user_level')." ORDER BY ss.`order`")->result_array();

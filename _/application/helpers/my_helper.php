...
if (!function_exists('simul_child')) {

    function simul_child($order){
        $CI = & get_instance();
        $sql = "SELECT * FROM simul_settings WHERE type = 'menu' AND stype = 'smain' AND `order` ='$order'";
...
if (!function_exists('simul_child_category')) {

    function simul_child_category(){
        $CI = & get_instance();
        $sql = "SELECT type FROM data_dashboard_list WHERE active = 1 GROUP BY type ORDER BY type ASC";
        $data = $CI->db->query($sql)->result_array();
        return $data;
    }
}

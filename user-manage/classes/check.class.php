...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['commo']['user_id'] );

		if ( empty($_SESSION['commo']['user_id']) ) :
			...
			$_SESSION['commo']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isActivated() {
		...
		if ( !empty($_SESSION['commo']['activate']) ) :
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['commo']['restricted']) || !empty($_SESSION['commo']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['commo']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['commo']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['commo']['user_id'] ) && $level == array('*') )
			return false;

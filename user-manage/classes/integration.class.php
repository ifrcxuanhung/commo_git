...
class Jigowatt_integration extends Generic {
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['commo']['user_id'] );
	...
	public function link_account( $link, $login = false ) {
		...
		try{
			...
			if ( !empty( $provider_user_profile ) ) {
				$_SESSION['commo'][$link . 'Misc'] = (array) $provider_user_profile;
				$_SESSION['commo'][$link] = $provider_user_profile->identifier;
			}

			/** See if the link was successful. */
			if ( !empty( $_SESSION['commo'][$link] ) ) {
	...
	private function connect( $link ) {
		...
		$params = array(
			...
			':session_link' => $_SESSION['commo'][$link]
		);

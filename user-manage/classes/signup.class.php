...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		foreach ($socialLogin as $value) {
			if( !empty($_SESSION['commo'][$value . 'Misc']) ) {
				$misc = $_SESSION['commo'][$value . 'Misc'];
			}
		}
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			//$_SESSION['commo']['username'] = $this->settings[$this->username_type];

			/* Apply default user_level */
			//$_SESSION['commo']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['commo']['activate'] = 1;

			//$_SESSION['commo']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
...
//echo "<pre>";print_r($params);exit;
			...
			/* Social integration. */
			if ( !empty($_SESSION['commo']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['commo']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['commo']['openIDMisc']) ) {
				$link = $_SESSION['commo']['openIDMisc']['type'];
				$id = $_SESSION['commo']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['commo']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['commo']['twitterMisc']['id'];
			}

...
class Login extends jigowatt_integration {
	...
	function __construct() {
		...
		if ( parent::getOption('signin-redirect-referrer-enable') )
			$_SESSION['commo']['referer'] = (!empty($_SESSION['commo']['referer'])) ? $_SESSION['commo']['referer'] : 'home.php';
		else
			$_SESSION['commo']['referer'] = parent::getOption('signin-redirect-url');
		...
		foreach (parent::$socialLogin as $provider) :
			if (!empty($_SESSION['commo'][$provider])) {
	...
	private function social_login($provider) {
		...
		$params = array( ':session' => $_SESSION['commo'][$provider] );
	...
	public function login() {
		...
		if ( parent::getOption('user-activation-enable') ) :
			...
			if ($count > 0) $_SESSION['commo']['activate'] = 1;

		endif;

		// Save if user is restricted
		if ( !empty($this->result['restricted']) ) $_SESSION['commo']['restricted'] = 1;
		...
		if (parent::getOption('pw-encrypt-force-enable')) :
			...
			if (strlen($this->result['password']) == 32 && $type == 'SHA256')
				$_SESSION['commo']['forcePwUpdate'] = 1;
		...
		$_SESSION['commo']['user_level'] = $user_level;

		$_SESSION['commo']['email'] = $this->result['email'];

		$_SESSION['commo']['gravatar'] = parent::get_gravatar($this->result['email'], true, 26);
		...
		if ( !empty($disRow['level_disabled']) ) $_SESSION['commo']['level_disabled'] = 1;
		...
		$_SESSION['commo']['username'] = $this->result[$username];

		// User ID of the logging in user
		$_SESSION['commo']['user_id'] = $this->result['user_id'];
		//$_SESSION['commo']['level_level'] = $this->result['level_level'];

		if ( empty($redirect) ) $redirect = $_SESSION['commo']['referer'];

		unset(
			$_SESSION['commo']['referer'],
			$_SESSION['commo']['token'],
			$_SESSION['commo']['facebookMisc'],
			$_SESSION['commo']['twitterMisc'],
			$_SESSION['commo']['openIDMisc']
		);

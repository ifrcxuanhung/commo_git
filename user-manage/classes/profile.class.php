...
class Profile extends Generic {
	...
	function __construct() {
		...
		if(!empty($_SESSION['commo']['forcePwUpdate']))
		...
		// Save the username
		$this->username = !empty($_SESSION['commo']['username']) ? $_SESSION['commo']['username'] : _('Guest');
		...
		if (!empty($_FILES['uploadAvatar'])) {
			...
			if (empty($k)) {
				...
				$uploadfile = $uploaddir . md5($_SESSION['commo']['user_id'] . $_SESSION['commo']['email']) . '.' . pathinfo($_FILES['uploadAvatar']['name'], PATHINFO_EXTENSION);
				if (move_uploaded_file($_FILES['uploadAvatar']['tmp_name'], $uploadfile)) {
					...
					$_SESSION['commo']['gravatar'] = parent::get_gravatar($_SESSION['commo']['email'], true, 26);
				} else {
					$this->error = sprintf('<div class="alert alert-warning">%s</div>', _('Sorry, that file is not accepted.') );
				}
			}
	...
	private function determineGuest() {
		...
		if ( !empty($_SESSION['commo']['user_id']) && empty($_GET['uid']) )
			$this->user_id = $_SESSION['commo']['user_id'];
		...
		$this->guest = !( !empty($_SESSION['commo']['user_id']) && $_SESSION['commo']['user_id'] == $this->user_id );
	...
	private function updateEmailorPw() {
		...
		if(!empty($_SESSION['commo']['forcePwUpdate'])) unset($_SESSION['commo']['forcePwUpdate']);

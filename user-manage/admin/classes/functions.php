...
if ( !empty($_POST['showUsers']) )
	$_SESSION['commo']['users_page_limit'] = $_POST['showUsers'];

if ( !empty($_POST['showLevels']) )
	$_SESSION['commo']['levels_page_limit'] = $_POST['showLevels'];
...
function pagination($table, $args = '',$total_pages = '') {
	...
	if($table == 'login_levels') :
		$hash = '#level-control';
		if (!empty($_SESSION['commo']['levels_page_limit']))
			$limit = $_SESSION['commo']['levels_page_limit'];
	endif;

	if($table == 'login_users') :
		$hash = '#user-control';
		if (!empty($_SESSION['commo']['users_page_limit']))
			$limit = $_SESSION['commo']['users_page_limit'];
	endif;
